...
<a href="{{ route('pemasukan.create') }}" accesskey="n" class="btn btn-primary pull-right">
  {{ trans('general.create') }}6xx
</a>
@endcan  --}}
@if (Auth::user()->isSuperUser() || Auth::user()->isAdmin())
@if(request('status') === 'Deleted')
    {{-- TAMPILKAN TOMBOL BACK --}}
    <a href="{{ route('pengeluaran.index') }}" class="btn btn-primary pull-right">
        Pengeluaran
    </a>
@else
      ...
      <div class="box-body">
        ...
        <div class="row" style="margin-bottom: 15px;">
          <div class="col-md-2">
            ...
            @endif
            @include('partials.pengeluaran-bulk-actions', ['status' => request('status')])
          </div>
        </div>
        <table
...
$(function() {
  $('#applyFilter').on('click', function() {
    ...
    var fileName = 'export-pengeluaran';
