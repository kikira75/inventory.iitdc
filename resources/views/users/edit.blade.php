            ...
            <div class="row">
              <div class="col-md-12">
                ...
                <div class="form-group {{ $errors->has('no_wa') ? 'has-error' : '' }}">
                  <label class="col-md-3 control-label" for="no_wa">whatsapp number</label>

                  <div class="col-md-6{{  (Helper::checkIfRequired($user, 'no_wa')) ? ' required' : '' }}">
                    @if ($user->ldap_import!='1' || str_contains(Route::currentRouteName(), 'clone'))
                      <input
                        class="form-control"
                        type="text"
                    ...
                    @else
                        <!-- insert the old no_wa so we don't break validation -->
                         {{ trans('general.managed_ldap') }}
                          <input type="hidden" name="no_wa" value="{{ Request::old('no_wa', $user->no_wa) }}">
                    @endif
                  </div>


                    @if (config('app.lock_passwords') && ($user->id))
                        <!-- disallow changing existing no_was on the demo -->
                        <div class="col-md-8 col-md-offset-3">
                            <p class="text-warning"><i class="fas fa-lock"></i> {{ trans('general.feature_disabled') }}</p>
                        </div>
                    @endif

                    @if ($errors->first('no_wa'))
                        <div class="col-md-8 col-md-offset-3">
                            {!! $errors->first('no_wa', '<span class="alert-msg" aria-hidden="true">:message</span>') !!}
                        </div>
                    @endif
