...
class StockOpnameController extends Controller
{
    ...
    public function bulkActions(Request $request)
    {
        ...
        if ($action === 'delete') {
            app()->instance('asset_action_context', 'stockopname_delete');
        
            // ambil semua asset_id dari baris yg dihapus
            $items = StockOpname::whereIn('id', $ids)->get();
            $assetIds = $items->pluck('asset_id')->unique();
        
            // delete data
            StockOpname::whereIn('id', $ids)->delete();
        
            // paksa trigger observer: update dummy field
            foreach ($assetIds as $assetId) {
                $asset = Asset::find($assetId);
                if ($asset) {
                    $asset->updated_at = now();
                    $asset->save(); // <--- ini yang memicu AssetObserver
                }
            }
        ...
        if ($action == 'restore') {
            app()->instance('asset_action_context', 'stockopname_restore');
