...
class AssetsController extends Controller
{
    ...
    public function index(Request $request, $audit = null) 
    {
        ...
        if ($request->filled('kategori_barang')) {
            $assets->where('kategori_barang', '=', $request->input('kategori_barang'));
        }

        if ($request->filled('saldo_awal')) {
            $assets->where('saldo_awal', '=', $request->input('saldo_awal'));
        }

        if ($request->filled('penyesuaian')) {
            $assets->where('penyesuaian', '=', $request->input('penyesuaian'));
        }

        if ($request->filled('saldo_buku')) {
            $assets->where('saldo_buku', '=', $request->input('saldo_buku'));
        }

        if ($request->filled('stock_opname')) {
            $assets->where('stock_opname', '=', $request->input('stock_opname'));
        }

        if ($request->filled('selisih')) {
            $assets->where('selisih', '=', $request->input('selisih'));
        }

        if ($request->filled('pemasukan')) {
            $assets->where('pemasukan', '=', $request->input('pemasukan'));
        }

        if ($request->filled('pengeluaran')) {
            $assets->where('pengeluaran', '=', $request->input('pengeluaran'));
        }

        if ($request->filled('jumlah_barang')) {
            $assets->where('jumlah_barang', '=', $request->input('jumlah_barang'));
        }

        if ($request->filled('satuan_barang')) {
            $assets->where('satuan_barang', '=', $request->input('satuan_barang'));
        }

        if ($request->filled('harga_satuan_barang')) {
            $assets->where('harga_satuan_barang', '=', $request->input('harga_satuan_barang'));
        }
