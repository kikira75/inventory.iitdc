...
class AssetsTransformer
{
    ...
    public function transformAsset(Asset $asset)
    {
        ...
        if ($asset->ppn) {
            $pajak[] = 'Tidak Dipungut PPN';
        }

        if ($asset->pph_impor) {
            $pajak[] = 'Tidak Dipungut PPh Impor';
        }

        if ($asset->ppnbm) {
            $pajak[] = 'Tidak Dipungut PPnBM';
        }
        $bea_radio = '';
        if (!empty($asset->bea)) {
            // Misalnya nilainya sudah tersimpan sebagai teks lengkap
            $bea_radio = $asset->bea; 
        }
        $pajak_text = implode(', ', $pajak);

        $bea_text = '';
        if ($bea_radio && $pajak_text) {
            $bea_text = 'Bea (' . $bea_radio . ') & Pajak (' . $pajak_text . ')';
        } elseif ($bea_radio) {
            $bea_text = 'Bea (' . $bea_radio . ')';
        } elseif ($pajak_text) {
            $bea_text = 'Pajak (' . $pajak_text . ')';
        }
        ...
        $array = [
            ...
            'kategori_barang' => (String) $asset->kategori_barang,
            'jumlah_barang' => (int) $asset->jumlah_barang,
            'mata_uang' => $asset->mata_uang,
            'satuan_barang' => (String) $asset->satuan_barang,
            'harga_satuan_barang' => (int) $asset->harga_satuan_barang,
            'harga_total_barang' => (int) $asset->harga_total_barang,
            'owner' => (String) $asset->owner,
            'departemen' => (String) $asset->departemen,
            'site' => (String) $asset->site,
            'lokasi_asset' => (String) $asset->lokasi_asset,
            'nama_dokumen_pabean' => (String) $asset->nama_dokumen_pabean,
            'kode_dokumen_pabean' => (String) $asset->kode_dokumen_pabean,
            'jumlah_baris_kode' => (String) $asset->jumlah_baris_kode,
            'jumlah_baris_pengeluaran' => (String) $asset->jumlah_baris_pengeluaran,
            'jumlah_baris_adjustment' => (String) $asset->jumlah_baris_adjustment,
            'jumlah_baris_stockopname' => (String) $asset->jumlah_baris_stockopname,
            'detail_lokasi_asset' => (String) $asset->detail_lokasi_asset,
            'status_pemasukan_mgpa' => (String) $asset->status_pemasukan_mgpa,
            'event' => (String) $asset->event,
            'metode_pengadaan' => (String) $asset->metode_pengadaan,
            'nomor_purchase' => (String) $asset->nomor_purchase,
            'keterangan' => (String) $asset->keterangan,
            'jenis_transaksi' => (String) $asset->jenis_transaksi,
            'saldo_awal' => (int) $asset->saldo_awal,
            'penyesuaian' => (int) $asset->penyesuaian,
            'saldo_buku' => (int) $asset->saldo_buku,
            'stock_opname' => (int) $asset->stock_opname,
            'selisih' => (int) $asset->selisih,
            'pemasukan' => (int) $asset->pemasukan,
            'pengeluaran' => (int) $asset->pengeluaran,
            'hasil_pencacahan' => (int) $asset->hasil_pencacahan,
            'jumlah_selisih' => (int) $asset->jumlah_selisih,
            'lampiran_display' => $asset->lampiran_display ?? '-',
        ];
        ...
        if(Auth::user()->isSuperUser() || Auth::user()->isAdmin()){
            $permissions_array['available_actions'] = [
                ...
                // 'restore'       => ($dat == 1 && Gate::allows('create', Asset::class)) ? true : false,
                'pemasukan'       => (Gate::allows('pemasukan', Asset::class)) ? true : false,
                'pengeluaran'     => (Gate::allows('pengeluaran', Asset::class)) ? true : false,
                'stockOpname'     => (Gate::allows('stockOpname', Asset::class)) ? true : false,
                'adjusment'       => (Gate::allows('adjusment', Asset::class)) ? true : false,
                'update'          => ($asset->deleted_at=='' && Gate::allows('update', Asset::class)) ? true : false,
                'delete'          => ($asset->deleted_at=='' && $asset->assigned_to =='' && Gate::allows('delete', Asset::class) && ($asset->deleted_at == '')) ? true : false,
            ];      
        }else if(Auth::user()->isLogistik() || Auth::user()->isAdminInputData()){
            $permissions_array['available_actions'] = [
                'pemasukan'       => (Gate::allows('getPemasukan', Asset::class)) ? true : false,
                'pengeluaran'     => (Gate::allows('create', Asset::class)) ? true : false,
                'stockOpname'     => (Gate::allows('create', Asset::class)) ? true : false,
                'adjusment'       => (Gate::allows('create', Asset::class)) ? true : false,
            ];
        }else if(Auth::user()->isAdminView()){
            $permissions_array['available_actions'] = [
                'pemasukan'       => false,
                'pengeluaran'     => false,
                'stockOpname'     => false,
