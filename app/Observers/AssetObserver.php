...
class AssetObserver
{
    ...
    {
        Log::info('=== AssetObserver::updating terpanggil ===', [
            'id' => $asset->id,
	    ...
	    if (empty($changed)){
            ...
            $context = app()->bound('asset_action_context') ? app()->make('asset_action_context') : null;

            if ($context === 'adjustment') {
                $logAction->logaction('adjustment');
            } elseif ($context === 'stock_opname') {
                $logAction->logaction('stock_opname');
            } elseif ($context === 'pemasukan') {
                $logAction->logaction('pemasukan');
            } elseif ($context === 'stockopname_restore') {
                $logAction->logaction('stockopname_restore');
            } elseif ($context === 'stockopname_delete') {
                $logAction->logaction('stockopname_delete');
            } else {
                $logAction->logaction('update9');
            }
            // $logAction->logaction('update99');
        }
    }
