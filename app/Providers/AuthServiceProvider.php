...
class AuthServiceProvider extends ServiceProvider
{
    ...
    protected $policies = [
        ...
        Pemasukan::class => PemasukanPolicy::class,
    ];
    ...
    public function boot()
    {
        ...
        Gate::define('a_logistik', function ($user) {
            if ($user->hasAccess('a_logistik')) {
                return true;
            }
        });

        Gate::define('a_inputdata', function ($user) {
            if ($user->hasAccess('a_inputdata')) {
                return true;
            }
        });
        ...
        // Gate::define('pemasukan', function ($user, $asset) {
        //     return $user->hasAccess('pemasukan', $asset);
        // });
        
        Gate::define('assets.pemasukan', function ($user) {
            if ($user->hasAccess('assets.pemasukan')) {
                return true;
            }
        });

        // Gate::define('pengeluaran', function ($user) {
        //     if ($user->hasAccess('pengeluaran')) {
        //         return true;
        //     }
        // });
        // Gate::define('stockOpname', function ($user) {
        //     if ($user->hasAccess('stockOpname')) {
        //         return true;
        //     }
        // });
        // Gate::define('adjusment', function ($user) {
        //     if ($user->hasAccess('adjusment')) {
        //         return true;
        //     }
        // });
        ...
        Gate::define('backend.interact', function ($user) {
            return $user->can('view', Statuslabel::class)
                ...
                || $user->can('view', Pemasukan::class)
                || $user->can('view', Pengeluaran::class)
                || $user->can('view', StockOpname::class)
                || $user->can('view', Adjustment::class)
        ...
        Gate::define('view.selectlists', function ($user) {
            return $user->can('update', Asset::class) 
                ...
                || $user->can('assets.pemasukan', Asset::class)       
                || $user->can('pengeluaran', Asset::class)       
                || $user->can('stockopname', Asset::class)       
                || $user->can('adjusment', Asset::class)       
                ...
                || $user->can('update', Pemasukan::class)
                || $user->can('create', Pemasukan::class)   
