...
class CronjobTransaksi extends Command
{
     ...
     * @var string
     */
    protected $signature = 'sg:cronjob-transaksi-pemasukan';
     ...
     * @var string
     */
    protected $description = 'Menambahkan Transaksi Pemasukan Otomatis';
     ...
     */
     ...
     public function handle(){
        ...
        $dataSend = Pemasukan::all()->where('status_sending', 'a');
        // dd($dataSend);
        ...
        if($dataSend != ""){
            ...
            try {
                ...
                echo 'Berhasil Mengirimkan Data Pemasukan Ke INSW dengan ' . count($dataResult) . ' Berhasil, dan ' . count($dataResultError) . ' data error - ' . now();
                ...
                echo $pesanError . ' -- Pada Data Pemasukan Ke = ' . $no . ' -- ' . now();

            }
        }else{
            echo 'Berhasil Mengirimkan Data Pemasukan Ke INSW dengan jumlah data = 0 - ' . now();
        }
    ...
    //     $asset = Asset::all()->where('status_pemasukan', 'B');
    //     $dt = Carbon::now();
    //     $data = [];
    //     $dataKet = [];
    ...
    //         $row = [];
    //         $dokument = [];
    //         $asset_pemasukan = $a['status_pemasukan'];

    //         $saldoAwal = $a["jumlah_barang"] * intval($a["harga_satuan_barang"]); 

    //         $row["kdKategoriBarang"] = (String) $a['nomor_kategori_barang'];
    //         $row["kdBarang"] = (String) $a["asset_tag"];
    //         $row["uraianBarang"] = (String) $a["name"];
    //         $row["jumlah"] = $a["jumlah_barang"];
    //         $row["kdSatuan"] = $a["satuan_barang"];
    //         $row["nilai"] = intval($saldoAwal);
    //         $row["dokumen"] = [];
    //         $data[] = $row;
    //     }

    //     $dataTransaksi = TransaksiPemasukan::all()->last();
    //     $noTrans = (int)  $dataTransaksi->transaksi_nomor;
    //     $nomorDokKegiatan = ucwords($dataTransaksi->transaksi_ket) . $noTrans;
    ...
    //     $dataHead["kdKegiatan"] = (String) config('app.kode_pemasukan'); 
    ...
    //             $transaksi = new TransaksiPemasukan();
    //             $transaksi->transaksi_nomor = $noTrans;
    //             $transaksi->transaksi_ket = 'Pemasukan';
    //             $transaksi->save();
